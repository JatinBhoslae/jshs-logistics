...
export default function ShipmentDetail() {
  ...
  const [editConsigneeName, setEditConsigneeName] = useState<string | null>(
    null,
  );
  const [editConsigneeContact, setEditConsigneeContact] = useState<
    string | null
  >(null);
  ...
  const simStatusQuery = useQuery({
    ...
    refetchInterval: (query) =>
      (query.state.data as any)?.running ? 10000 : false,
  ...
  useEffect(() => {
    ...
    const handler = (msg: {
      ...
      // Update the shipment data in the query cache if possible, or just let the next refetch handle it
      // For now, we can update the local state if we had one, but we are using useQuery.
      // The simulation emits these, so we should ideally update the query data.
  ...
  if (!shipment) {
    return <ErrorDisplay message="Shipment not found." className="m-4" />;
  }
  ...
  async function updateShipment() {
    ...
    if (etaLocal !== originalEta)
      payload.eta = etaLocal ? new Date(etaLocal).toISOString() : null;

    // Consignee Updates
    const currentConsignee: { name?: string; contact?: string } =
      shipment.consignee || {};
    const newConsignee: { name?: string; contact?: string } = {
      ...currentConsignee,
    };
    let consigneeChanged = false;

    if (
      editConsigneeName !== null &&
      editConsigneeName !== currentConsignee.name
    ) {
    ...
    if (
      editConsigneeContact !== null &&
      editConsigneeContact !== currentConsignee.contact
    ) {
  ...
  async function remove() {
    ...
    if (
      !confirm(
        "Delete this shipment? This will remove related pings/docs/invoices.",
      )
    )
      return;
    try {
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-6 text-left">
          <div className="grid grid-cols-1 gap-4">
            ...
            <button
              ...
              disabled={
                !(assignDriverId ?? shipment.assignedDriverId) ||
                !(assignVehicleId ?? shipment.assignedVehicleId)
              }
            >
      ...
      >
        <div className="space-y-4 py-4">
          <p className="text-sm text-slate-500">
            Please enter the 6-digit OTP provided by the{" "}
            {otpType === "START" ? "consignor" : "consignee"} to proceed.
          </p>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              Verification Code
      ...
      >
        <div className="space-y-4 py-4">
          <p className="text-xs text-slate-500 font-medium">
            Modify the destination contact person details. This will update the
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-white/10">
        <div className="flex items-center gap-5">
          <div
            className={`h-16 w-16 rounded-[1.5rem] flex items-center justify-center shadow-2xl ${
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <span
                className={`inline-flex items-center rounded-lg px-3 py-1 text-[10px] font-black uppercase tracking-widest ring-1 ring-inset ${
                  shipment.status === "DELIVERED"
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex gap-2 p-1.5 bg-slate-100 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/10">
              {["IN_TRANSIT", "DISPATCHED", "OUT_FOR_DELIVERY"].includes(
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all font-black text-[10px] uppercase tracking-widest border ${
                ...
                >
                  <Radio
                    className={`h-3.5 w-3.5 ${simStatusQuery.data?.running ? "animate-pulse" : ""}`}
                  />
          ...
          {user?.role === "DRIVER" &&
            String(shipment.assignedDriverId) === String(user.id) && (
              <div className="flex gap-2">
                {shipment.status === "ASSIGNED" && (
                  <>
                    <button
                ...
                {shipment.status === "PICKED_UP" && (
                  <button
                    ...
                    className="btn-primary bg-blue-600 hover:bg-blue-700 shadow-blue-500/20 flex items-center gap-2"
                  >
          ...
          {["CREATED", "ASSIGNED"].includes(shipment.status) &&
            (isManagerPortal ||
              (user?.role === "CUSTOMER" &&
                String(shipment.customerId) === String(user.id))) && (
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    toast.error(
                      err.response?.data?.error?.message || "Failed to cancel",
                    );
                  }
                }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-2">
                    {user?.role === "CUSTOMER" &&
                      [
                        ...
                        <button
                          onClick={() => {
                            setEditConsigneeName(
                              shipment.consignee?.name || "",
                            );
                            setEditConsigneeContact(
                              shipment.consignee?.contact || "",
                            );
          ...
          {(user?.role === "CUSTOMER" || isManagerPortal) &&
            shipment.assignedDriverId &&
            typeof shipment.assignedDriverId === "object" && (
              <div className="glass-card bg-slate-900 text-white border-none p-6 shadow-2xl relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start mb-4">
                  <label className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] block">
                    Assigned Logistics Elite
                  </label>
                  {reviewQuery.data && (
                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-amber-500/20 border border-amber-500/30 text-amber-400">
                      ...
                      <span className="text-[10px] font-black">
                        RATED {reviewQuery.data.rating}/5
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center hover:bg-white/10 transition-colors">
                    ...
                    <div className="text-sm font-black">
                      {(shipment.assignedDriverId as any).performanceRating ||
                        5}
                      /5
                    </div>
                  ...
                  <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center hover:bg-white/10 transition-colors">
                    ...
                    <div className="text-sm font-black">
                      {(shipment.assignedDriverId as any).yearsOfExperience ||
                        0}
                      y
                    </div>
                    <div className="text-[8px] font-bold text-slate-500 uppercase">
                  ...
                  <div className="bg-white/5 p-3 rounded-xl border border-white/10 text-center hover:bg-white/10 transition-colors">
                    <AlertTriangle
                      className={`h-4 w-4 mx-auto mb-1 ${(shipment.assignedDriverId as any).challansCount > 0 ? "text-rose-500" : "text-emerald-500"}`}
                    />
                    <div className="text-sm font-black">
          ...
          <div className="glass-card border-none shadow-2xl bg-white dark:bg-slate-900 overflow-hidden p-0">
            <div className="p-6 bg-slate-900 text-white">
              ...
              <p className="text-slate-400 text-xs font-bold leading-relaxed">
                Automated compliance tracking for global standards{" "}
                {shipment.referenceId}
              </p>
            </div>
            ...
            <div className="p-6 space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="h-7 w-7 rounded-full bg-blue-600 flex items-center justify-center text-[12px] font-black text-white shadow-lg shadow-blue-500/20">
                    1
                  </div>
                  <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">
                    Pre-Journey Dispatch
                  </h4>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <DocActionItem
                    ...
                    doc={docsQ.data?.find(
                      (d) => d.type === "DISPATCH_MANIFEST",
                    )}
                    onGenerate={() => generateDoc("DISPATCH_MANIFEST")}
                    onUpload={(file: File) =>
                      handleUploadDoc("DISPATCH_MANIFEST", file)
                    }
                  ...
                  <DocActionItem
                    ...
                    doc={docsQ.data?.find(
                      (d) => d.type === "VEHICLE_INSPECTION",
                    )}
                    onGenerate={() => generateDoc("VEHICLE_INSPECTION")}
                    onUpload={(file: File) =>
                      handleUploadDoc("VEHICLE_INSPECTION", file)
                    }
                    isGenerating={isGenerating}
                    isManagerPortal={isManagerPortal}
                  />
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="h-7 w-7 rounded-full bg-indigo-600 flex items-center justify-center text-[12px] font-black text-white shadow-lg shadow-indigo-500/20">
                    2
                  </div>
                  <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">
                    Transit Compliance
                ...
                <div className="grid grid-cols-1 gap-3">
                  <DocActionItem
                    ...
                    onUpload={(file: File) =>
                      handleUploadDoc("E_WAY_BILL", file)
                    }
                    isGenerating={isGenerating}
                    isManagerPortal={isManagerPortal}
                  />
                  <DocActionItem
                    ...
                    onUpload={(file: File) =>
                      handleUploadDoc("CONSIGNMENT_NOTE", file)
                    }
                    isGenerating={isGenerating}
                    isManagerPortal={isManagerPortal}
                  />
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="h-7 w-7 rounded-full bg-emerald-600 flex items-center justify-center text-[12px] font-black text-white shadow-lg shadow-emerald-500/20">
                    3
                  </div>
                  <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">
                    Delivery & Billing
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  <DocActionItem
                    ...
                    onUpload={(file: File) =>
                      handleUploadDoc("GST_INVOICE", file)
                    }
                    isGenerating={isGenerating}
                    isManagerPortal={isManagerPortal}
                  />
            ...
            <div className="p-6 bg-emerald-50 dark:bg-emerald-500/5 border-t border-emerald-100 dark:border-emerald-500/20 flex items-center justify-between">
              <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">
                {docsQ.data?.filter((d) => d.verified).length || 0} /{" "}
                {docsQ.data?.length || 0} VERIFIED
              </span>
            </div>
          </div>
        </div>
...
function DocActionItem({
  ...
  return (
    <div
      className={`flex items-center justify-between p-4 rounded-[1.5rem] border transition-all group ${
        doc
          ? "bg-white dark:bg-slate-900 border-slate-100 dark:border-white/5 shadow-sm"
          : "bg-slate-50/50 dark:bg-white/5 border-dashed border-slate-200 dark:border-white/10"
      }`}
    >
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="min-w-0">
          <div className="text-sm font-black text-slate-900 dark:text-white truncate tracking-tight">
            {label}
          </div>
          {doc ? (
      ...
      {!doc && isManagerPortal && (
        <div className="flex gap-2">
          <button
            ...
            className="h-9 w-9 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm flex items-center justify-center disabled:opacity-50"
            title="Auto-generate Professional PDF"
          >
            <Plus className="h-4 w-4" />
          </button>
          <label
            className="h-9 w-9 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-all shadow-sm flex items-center justify-center cursor-pointer"
            title="Manual Upload"
          >
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="glass-card h-20 flex flex-col justify-between"
              >
