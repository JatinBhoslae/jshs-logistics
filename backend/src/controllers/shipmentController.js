...
import {
  getVehicleHealthStatus,
  getFleetHealth,
} from "../services/gpsTrackingService.js";
...
const createSchema = z.object({
  ...
  shipmentType: z
    .enum(["KIRANA", "DAWAI", "KAPDA", "DAIRY", "AUTO_PARTS", "ELECTRONICS"])
    .optional(),
  ...
  consignee: z
    .object({
      ...
      contact: z
        ...
        .regex(
          /^\+91[6-9]\d{9}$/,
          "Invalid Indian 10-digit phone number. Must start with +91",
        )
        .optional(),
    })
    .optional(),
...
export async function listShipments(req, res) {
  ...
  if (role === "DRIVER") {
    if (req.user.driverApprovalStatus !== "APPROVED")
      return res.json({ shipments: [], total: 0, pages: 0, currentPage: 1 });
...
export async function estimateShipment(req, res) {
  ...
  if (!data.origin || !data.destination) {
    return res.status(400).json({
      error: {
        message: "Origin and Destination points are required for estimation.",
      },
    });
  }
  ...
  if (
    !isWithinIndia(data.origin.lat, data.origin.lng) ||
    !isWithinIndia(data.destination.lat, data.destination.lng)
  ) {
    return res.json({
      serviceable: false,
      message:
        "Service not available in the provided locations (Outside India).",
    });
  }
  ...
  estimated_eta_date.setHours(
    estimated_eta_date.getHours() + travel_hours + buffer,
  );
  ...
  res.json({
    ...
    isRemote,
    message: isRemote
      ? "Note: Destination is in a remote area. ETA has been extended."
      : undefined,
  });
}
...
export async function createShipment(req, res) {
  ...
  const distanceKm =
    Math.round(haversineKm(data.origin, data.destination) * 100) / 100;
  ...
  if (existing) {
    return res.status(409).json({
      error: {
        message:
          "A similar shipment was created in the last minute. Please check for duplicates.",
      },
    });
  }
  ...
  const refId =
    data.referenceId ||
    `CN-${Math.random().toString(16).slice(2, 8).toUpperCase()}`;
  ...
  await createShipmentNotification(
    shipment,
    "SHIPMENT_CREATED",
    `Shipment ${shipment.referenceId} has been created successfully. Tracking ID: ${shipment.referenceId}`,
  );

  // Simulate SMS/Email
  console.log(
    `[AUTOMATION] Sending SMS to customer ${shipment.customerId}: Your shipment ${shipment.referenceId} has been created. Track it here: http://localhost:5173/app/shipment/${shipment._id}`,
  );

  // Calculate optimized route async
  getDrivingRoute({ origin: data.origin, destination: data.destination })
    .then(async (route) => {
  ...
  if (data.driverId && data.vehicleId) {
    ...
    await createShipmentNotification(
      out,
      "SHIPMENT_ASSIGNED",
      `Shipment ${shipment.referenceId} has been assigned.`,
    );
  } else {
    // **AUTOMATION**: Always attempt auto-assignment on creation
    try {
      out = await autoAssignShipment({
        shipmentId: shipment._id,
        actorId: req.user._id,
      });
    } catch (e) {
  ...
  automateInvoicing(out, req.user).catch((err) =>
    console.error("Initial invoicing failed:", err.message),
  );
...
export async function getShipment(req, res) {
  ...
  if (req.user.role === "CUSTOMER") {
    query
      .populate(
        "assignedDriverId",
        "name phone performanceRating challansCount totalTrips yearsOfExperience",
      )
    ...
    query
      .populate(
        "assignedDriverId",
        "name email phone performanceRating licenseNumber totalTrips challansCount",
      )
      .populate("customerId", "name email phone legalName address");
  }
  ...
  if (!shipment.routeGeoJson) {
    try {
      ...
      await Shipment.findByIdAndUpdate(shipment._id, {
        $set: {
          ...
          routeDistanceKm: r.distanceKm
            ? Math.round(r.distanceKm * 100) / 100
            : undefined,
      ...
      shipment.routeDistanceKm = r.distanceKm
        ? Math.round(r.distanceKm * 100) / 100
        : undefined;
      shipment.routeDurationMin = r.durationMin
        ? Math.round(r.durationMin)
        : undefined;
  ...
  if (req.user.role === "DRIVER") {
    if (req.user.driverApprovalStatus !== "APPROVED")
      return res
        .status(403)
        .json({ error: { message: "Driver not approved" } });
    const driverId =
      shipment.assignedDriverId?._id || shipment.assignedDriverId;
...
export async function updateShipment(req, res) {
  ...
  if (data.origin || data.destination) {
    shipment.distanceKm =
      Math.round(haversineKm(shipment.origin, shipment.destination) * 100) /
      100;

    // Refresh predicted ETA based on current location (if any) else origin
    const from = shipment.currentLocation
      ? { lat: shipment.currentLocation.lat, lng: shipment.currentLocation.lng }
      : shipment.origin;
    shipment.predictedEta = estimateEta({
      from,
  ...
  if (shipment.status === "IN_TRANSIT" || shipment.status === "DISPATCHED") {
    const generateDispatchDocs = async () => {
      ...
      automateInvoicing(shipment, req.user).catch((err) =>
        console.error("Dispatch invoicing update failed:", err.message),
      );
    };
  ...
  if (shipment.status === "DELIVERED" || shipment.status === "CLOSED") {
    const generateDeliveryDocs = async () => {
      ...
      for (const type of types) {
        ...
        if (!existing) {
          ...
          const { fileName, relativePath, absolutePath } =
            await generateShipmentPdf({ shipment, type, actor, options });
          await Document.create({
            shipmentId: shipment._id,
            type,
            fileName,
      ...
      automateInvoicing(shipment, req.user).catch((err) =>
        console.error("Final invoicing failed:", err.message),
      );
    };
...
export async function dispatchShipment(req, res) {
  ...
  if (
    ...
    String(shipment.assignedDriverId ?? "") !== String(req.user._id)
  ) {
    return res.status(403).json({ error: { message: "Forbidden" } });
  }
  ...
  if (shipment.status !== "PICKED_UP") {
    return res.status(400).json({
      error: {
        message: "Shipment must be PICKED_UP before it can be dispatched",
      },
    });
  }
...
export async function deliverShipment(req, res) {
  try {
    ...
    if (!otp)
      return res
        .status(400)
        .json({ error: { message: "Delivery completion OTP is required" } });

    console.log(
      `[OTP] Verifying COMPLETE OTP for shipment ${shipment._id}. Input: ${otp}, Stored: ${shipment.deliveryCompleteOtp}`,
    );

    if (shipment.deliveryCompleteOtp !== otp) {
      console.log(
        `[OTP] COMPLETE OTP verification failed for shipment ${shipment._id}. Input: ${otp}, Stored: ${shipment.deliveryCompleteOtp}`,
      );
      return res
        .status(400)
        .json({ error: { message: "Invalid delivery completion OTP" } });
    }
    ...
    if (!shipment.driverEarnings?.amount) {
      const earnings = (shipment.price || 0) * 0.7;
    ...
    await createShipmentNotification(
      shipment,
      "SHIPMENT_DELIVERED",
      `Shipment ${shipment.referenceId} has been successfully delivered.`,
    );
    ...
    if (shipment.assignedVehicleId) {
      ...
      if (v) {
        ...
        v.odometerKm =
          Math.round(((v.odometerKm || 0) + distanceGained) * 100) / 100;

        // Calculate distance since last service
        const distanceSinceService =
          v.odometerKm - (v.lastServiceOdometerKm || 0);
    ...
    // 🚀 Background Tasks: Auto-generate delivery paperwork
    const generateDeliveryDocs = async () => {
      try {
        // Ensure we have full details for the Invoice
        const fullShipment = await Shipment.findById(shipment._id).populate(
          "customerId assignedDriverId assignedVehicleId",
        );
        ...
        for (const type of types) {
          ...
          if (!existing) {
            ...
            let options = {};
            if (type === "GST_INVOICE") {
              const refPart = (fullShipment.referenceId || "0000").slice(0, 4);
              const namePart = (
                fullShipment.customerId?.legalName ||
                fullShipment.customerId?.name ||
                "CUST"
              )
                .replace(/\s+/g, "")
                .slice(0, 4);
              ...
              console.log(
                `[INVOICE] Generating password protected PDF. Password: ${password}`,
              );
            }
            ...
            await Document.create({
              shipmentId: fullShipment._id,
              customerId: fullShipment.customerId?._id,
              driverId: fullShipment.assignedDriverId?._id,
              vehicleId: fullShipment.assignedVehicleId?._id,
              type,
              fileName,
              filePath: relativePath,
              uploadedById: req.user._id,
            ...
            // Email Trigger for Invoice
            if (type === "GST_INVOICE" && fullShipment.customerId?.email) {
              await sendInvoiceEmail(
                fullShipment.customerId,
                fullShipment,
                absolutePath,
                options.password,
              );
        ...
        console.error(
          "[DOC-GEN] Background doc generation failed:",
          err.message,
        );
      }
    };
    generateDeliveryDocs(); // Fire and forget
...
export async function getVehicleRealtimePosition(req, res) {
  ...
  const latestPing = await LocationPing.findOne({ vehicleId })
    .sort({ ts: -1 })
    .lean();
  if (!latestPing)
    return res
      .status(404)
      .json({ error: { message: "No location data found" } });
...
export async function getShipmentRealtimeTracking(req, res) {
  ...
  const latestPing = await LocationPing.findOne({ shipmentId })
    .sort({ ts: -1 })
    .lean();
  const locationHistory = await LocationPing.find({ shipmentId })
    .sort({ ts: 1 })
    .limit(100)
    .lean();
  ...
  if (!progressPercentage && shipment.distanceKm && latestPing) {
    const distanceTraveled = haversineKm(shipment.origin, {
      lat: latestPing.lat,
      lng: latestPing.lng,
    });
    progressPercentage = Math.min(
      100,
      Math.round((distanceTraveled / shipment.distanceKm) * 100),
    );
  }
  ...
  res.json({
    ...
    recentEvents: await ShipmentEvent.find({ shipmentId })
      .sort({ createdAt: -1 })
      .limit(20)
      .lean(),
  });
...
export async function getFleetRealtimePositions(req, res) {
  ...
  for (const v of vehicles) {
    ...
    let latestPing = await LocationPing.findOne({ vehicleId: v._id })
      .sort({ ts: -1 })
      .lean();
    ...
    if (!latestPing && v.status === "IN_USE") {
      const activeShipment = await Shipment.findOne({
        ...
        status: {
          $in: ["PICKED_UP", "IN_TRANSIT", "DELAYED", "OUT_FOR_DELIVERY"],
        },
      }).lean();
    ...
    if (latestPing) {
      ...
      vehiclePositions.push({
        vehicle: v,
        position: { lat: 28.6139, lng: 77.209 }, // Default Base: New Delhi
        lastUpdate: new Date(),
        status: v.status,
        isSimulated: true,
      });
...
export async function markShipmentLoaded(req, res) {
  try {
    ...
    await createShipmentNotification(
      shipment,
      "SHIPMENT_PICKED_UP",
      `Shipment ${shipment.referenceId} has been picked up.`,
    );
...
export async function uploadProofOfLoading(req, res) {
  if (!req.file) {
    return res
      .status(400)
      .json({ error: { message: "Proof of loading document is required" } });
  }
  ...
  await createShipmentNotification(
    shipment,
    "PROOF_OF_LOADING_UPLOADED",
    `Proof of loading for shipment ${shipment.referenceId} has been uploaded.`,
  );
...
export async function addDriverEsign(req, res) {
  ...
  await createShipmentNotification(
    shipment,
    "DRIVER_ESIGN_ADDED",
    `Driver has added e-signature for shipment ${shipment.referenceId}.`,
  );
...
export async function verifyProofOfLoading(req, res) {
  ...
  await createShipmentNotification(
    shipment,
    "PROOF_OF_LOADING_VERIFIED",
    `Proof of loading for shipment ${shipment.referenceId} has been verified.`,
  );
...
export async function startShipment(req, res) {
  ...
  if (shipment.status !== "PICKED_UP") {
    return res.status(400).json({
      error: {
        message: "Shipment must be PICKED_UP before starting journey",
      },
    });
  }
  ...
  */
  ...
  if (!otp)
    return res
      .status(400)
      .json({ error: { message: "Delivery start OTP is required" } });

  console.log(
    `[OTP] Verifying START OTP for shipment ${shipment._id}. Input: ${otp}, Stored: ${shipment.deliveryStartOtp}`,
  );

  if (shipment.deliveryStartOtp !== otp) {
    console.log(
      `[OTP] START OTP verification failed for shipment ${shipment._id}. Input: ${otp}, Stored: ${shipment.deliveryStartOtp}`,
    );
    return res
      .status(400)
      .json({ error: { message: "Invalid delivery start OTP" } });
  }
  ...
  await createShipmentNotification(
    shipment,
    "SHIPMENT_STARTED",
    `Shipment ${shipment.referenceId} has started its journey.`,
  );
...
export async function acceptShipmentAssignment(req, res) {
  ...
  if (String(shipment.assignedDriverId ?? "") !== String(req.user._id)) {
    return res.status(403).json({
      error: { message: "Forbidden - Not assigned to this shipment" },
    });
  }
  ...
  if (shipment.status !== "ASSIGNED") {
    return res
      .status(400)
      .json({ error: { message: "Shipment is not pending acceptance" } });
  }
  ...
  await createShipmentNotification(
    shipment,
    "SHIPMENT_ACCEPTED",
    `Driver accepted assignment for shipment ${shipment.referenceId}.`,
  );
...
export async function rejectShipmentAssignment(req, res) {
  ...
  try {
    const { autoAssignShipment } =
      await import("../services/shipmentService.js");
    ...
    return res.json({
      ...
      message:
        "Assignment rejected. Shipment reassigned to next available driver.",
    });
  } catch (reassignErr) {
    ...
    return res.json({
      shipment,
      message:
        "Assignment rejected. No other drivers available. Manager has been notified.",
...
export async function cancelShipment(req, res) {
  ...
  if (!["CREATED", "ASSIGNED"].includes(shipment.status)) {
    return res.status(400).json({
      error: {
        message: `Cannot cancel shipment in ${shipment.status} status. Only CREATED or ASSIGNED shipments can be cancelled.`,
      },
    });
  }
  ...
  if (
    req.user.role === "CUSTOMER" &&
    String(shipment.customerId ?? "") !== String(req.user._id)
  ) {
    return res.status(403).json({ error: { message: "Forbidden" } });
  }
  ...
  await createShipmentNotification(
    shipment,
    "SHIPMENT_CANCELLED",
    `Shipment ${shipment.referenceId} has been cancelled.`,
  );
...
export async function requestShipmentOtp(req, res) {
  try {
    ...
    // Clear any existing OTPs first to prevent conflicts
    if (type === "START") {
      shipment.deliveryStartOtp = otp;
      shipment.deliveryCompleteOtp = null; // Clear completion OTP to avoid conflicts
    } else if (type === "COMPLETE") {
      shipment.deliveryCompleteOtp = otp;
      shipment.deliveryStartOtp = null; // Clear start OTP to avoid conflicts
    } else {
    ...
    console.log(
      `[OTP] Generated ${type} OTP for Shipment ${shipment.referenceId}: ${otp}`,
    );
    ...
    if (type === "START") {
      ...
      if (customer) {
        ...
        if (customer.phone) {
          ...
          // The sendSMS service already redirects to +919421893340
          const ok = await sendSMS(customer.phone, body);
          if (ok) transmissionStatus = true;
        }
      }
    } else if (type === "COMPLETE") {
      // 2. Send to Consignee Phone
    ...
    res.json({
      success: true,
      message: `OTP generated. For demo purposes, it has been redirected to the Master Demo Number (${MASTER_DEMO_NUMBER}).`,
    });
  } catch (err) {
  ...
  try {
    ...
    switch (docType) {
      ...
      default:
        return res
          .status(400)
          .json({ error: { message: "Invalid document type for automation" } });
    }
