...
export const createReview = async (req, res) => {
  try {
    ...
    if (shipment.customerId.toString() !== customerId.toString()) {
      return res
        .status(403)
        .json({ message: "Not authorized to review this shipment" });
    }
    ...
    if (shipment.status !== "DELIVERED" && shipment.status !== "CLOSED") {
      return res
        .status(400)
        .json({ message: "Shipment must be delivered before reviewing" });
    }

    if (shipment.paymentStatus !== "PAID") {
      return res
        .status(400)
        .json({ message: "Payment must be completed before reviewing" });
    }

    // 2.5 Ensure driver is assigned
    if (!shipment.assignedDriverId) {
      return res
        .status(400)
        .json({
          message: "Cannot review a shipment without an assigned driver",
        });
    }
    ...
    if (existingReview) {
      return res
        .status(400)
        .json({ message: "Review already submitted for this shipment" });
    }
    ...
    if (driver) {
      ...
      if (
        avgRating >= 4.5 &&
        allReviews.length >= 5 &&
        !driver.awards?.includes("Top Rated Driver")
      ) {
