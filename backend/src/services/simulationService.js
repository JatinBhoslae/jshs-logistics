  ...
  simulationInterval = setInterval(async () => {
    try {
      ...
      for (const s of shipments) {
        ...
        if (
          !s.routeGeoJson ||
          !s.routeGeoJson.coordinates ||
          s.routeGeoJson.coordinates.length < 2
        ) {
          try {
            ...
            console.error(
              `[SIMULATION] Route calc failed for ${s.referenceId}:`,
              err.message,
            );
        ...
        // Real-time ETA calculation based on simulation progress
        const totalPoints = routePoints.length;
        const remainingPoints = totalPoints - nextIdx;
        const distanceRemaining = s.distanceKm
          ? (remainingPoints / totalPoints) * s.distanceKm
          : haversineKm({ lat: newLat, lng: newLng }, s.destination);

        // Assuming average speed of 40 km/h for simulation
        const avgSpeed = 40;
        const newEta = estimateEta({
          from: { lat: newLat, lng: newLng },
          to: s.destination,
          speedKmph: avgSpeed,
        });
        ...
        s.distanceRemainingKm = Math.round(distanceRemaining * 100) / 100;
        s.predictedEta = newEta;
        s.predictedEtaUpdatedAt = new Date();

        // Progress
        const progress = Math.min(
          100,
          Math.round((nextIdx / routePoints.length) * 100),
        );
        s.progressPercentage = progress;

        await s.save();

        // Create a LocationPing for history and persistence
        ...
        if (s.assignedVehicleId) {
          ...
          if (vehicle) {
            ...
            vehicle.currentFuelLiters = Math.max(
              0,
              vehicle.currentFuelLiters - fuelConsumed,
            );

            // 3. Simulate Chiller Temperature (if refrigerated)
            if (vehicle.isRefrigerated) {
              if (!vehicle.currentTemperatureC)
                vehicle.currentTemperatureC = -20;
              // Small fluctuation
              ...
              if (
                vehicle.currentTemperatureC > vehicle.temperatureThresholdMaxC
              ) {
                ...
                if (manager) {
                  await createNotification({
                    ...
                    metadata: {
                      ...
                      temperature: vehicle.currentTemperatureC,
                    },
                  });
                }
              }
